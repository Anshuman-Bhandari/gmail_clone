...
const SendEmail = () => {
    ...
    const submitHandler = async(e) => {
        e.preventDefault()
        try {
            const res = await axios.post('https://gmail-clone-cqdy.onrender.com/api/v1/email/create', formData, {
                headers: {
  ...
  return (
    <div className={`${open ? 'block' : 'hidden'} bg-white max-w-6xl shadow-xl shadow-slate-600 rounded-t-md`}>
        ...
        <form onSubmit={submitHandler} className='flex flex-col p-3 gap-2'>
            ...
            <button type='submit' className='bg-blue-500 rounded-2xl px-5 py-1 w-fit text-white hover:cursor-pointer'>Send</button>
        </form>
    </div>
  )
}
